...
function setEmotion(emotion) {
	fetch(`${origin}/apps/faces/emotion`, {method: "put", body: emotion})
	renderEmotionAsCurrentEmotion(emotion)
}
function getCurrentEmotion(callback) {
	fetch(`${origin}/apps/faces/emotion`, {method: "get"})
		.then(response => response.text())
		.then(emotion_text => callback(emotion_text))
}
function getPossibleEmotions() {
	return fetch(`${origin}/apps/faces/emotion/all`, {method: "get"})
		.then(response => response.text())
		.then(text => {
...
getPossibleEmotions().then(possible_emotions => {
	for(let emotion of possible_emotions) {
		...
		button_image.setAttribute("src", `${origin}/apps/faces/static/protogen_images/eyes/${emotion}/thumbnail.png`)
...
function setScreenBlank(blank) {
	fetch(`${origin}/apps/faces/blank`, {method: "put", body: blank})
}
function getScreenBlank(callback) {
	fetch(`${origin}/apps/faces/blank`, {method: "get"})
		.then(response => response.text())
...
function setAudioLevel(level) {
	fetch(`${origin}/apps/faces/audio-loudness`, {method: "put", body: level})
}
